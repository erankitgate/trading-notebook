...
"""
import json, sys, time, urllib.request, urllib.error, urllib.parse, datetime as dt
...
NIFTY50 = [
    ...
    "SHRIRAMFIN", "SUNPHARMA", "TATACONSUM", "TMCV", "TMPV", "TATASTEEL", "TCS", "TECHM", "TITAN", "TRENT", "ULTRACEMCO", "WIPRO",
]
IST = dt.timezone(dt.timedelta(hours=5, minutes=30))


PAUSE = 1.2  # seconds between requests; Yahoo returns 429 when hammered
...
def fetch(symbol, rng="1y"):
    """Full daily candles for one symbol: [(ts, o, h, l, c, v)], meta."""
    host = "query2" if hash(symbol) % 2 else "query1"
    d = _get(f"https://{host}.finance.yahoo.com/v8/finance/chart/{urllib.parse.quote(symbol)}?range={rng}&interval=1d")["chart"]["result"][0]
    q, m = d["indicators"]["quote"][0], d["meta"]
    rows = [(t, o, h, l, c, v) for t, o, h, l, c, v in zip(d["timestamp"], q["open"], q["high"], q["low"], q["close"], q["volume"]) if c is not None]
    # Today's candle is published with a blank close until settlement; the meta carries the last price.
    if d["timestamp"] and q["close"][-1] is None and m.get("regularMarketPrice") and (not rows or d["timestamp"][-1] > rows[-1][0]):
        t = d["timestamp"][-1]
        rows.append((t, q["open"][-1] or m.get("regularMarketPrice"), max(q["high"][-1] or 0, m.get("regularMarketDayHigh") or 0), min(x for x in [q["low"][-1], m.get("regularMarketDayLow")] if x), m["regularMarketPrice"], q["volume"][-1] or 0))
    return rows, m


def fetch_many(symbols, rng="1y"):
    """Closes only, many symbols per request (the spark endpoint): {symbol: [(ts, c)]}."""
    out = {}
    for i in range(0, len(symbols), 20):
        chunk = symbols[i:i + 20]
        d = _get(f"https://query1.finance.yahoo.com/v7/finance/spark?symbols={urllib.parse.quote(','.join(chunk))}&range={rng}&interval=1d")
        for r in d.get("spark", {}).get("result", []):
            resp = r["response"][0]
            closes, m = resp["indicators"]["quote"][0]["close"], resp.get("meta", {})
            pts = [(t, c) for t, c in zip(resp["timestamp"], closes) if c is not None]
            if resp["timestamp"] and closes[-1] is None and m.get("regularMarketPrice") and (not pts or resp["timestamp"][-1] > pts[-1][0]):
                pts.append((resp["timestamp"][-1], m["regularMarketPrice"]))
            out[r["symbol"]] = (pts, m)
    return out
...
def analyse_closes(symbol, label, pts, meta):
...
def main():
    ...
    try:
        got = fetch_many([sym for sym, _ in GLOBALS], "1y")
        for sym, label in GLOBALS:
            if sym in got and got[sym][0]:
                out["globals"].append(analyse_closes(sym, label, *got[sym]))
            else:
                out["failed"].append(f"{sym}: no data")
    except Exception as e:
        out["failed"].append(f"globals: {e}")
    try:
        got = fetch_many([f"{s}.NS" for s in NIFTY50], "1y")
        for s in NIFTY50:
            k = f"{s}.NS"
            if k in got and got[k][0]:
                out["stocks"].append(analyse_closes(s, s, *got[k]))
            else:
                out["failed"].append(f"{k}: no data")
    except Exception as e:
        out["failed"].append(f"stocks: {e}")
