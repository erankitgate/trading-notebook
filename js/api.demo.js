...
function build() {
  ...
  const series = []; { let c = 24300; for (let k = 90; k >= 0; k--) { c = +(c + Math.sin(k / 6) * 60 - 9 + (rnd() - 0.5) * 80).toFixed(1); const d = addDays(today(), -k - 1); if (weekday(d) !== 0 && weekday(d) !== 6) series.push([d, +(c - 30).toFixed(1), +(c + 90).toFixed(1), +(c - 110).toFixed(1), c]); } }
  const nifty = mk('^NSEI', 'Nifty 50', 23414.3, 0.29, 58.2, 'up', { sma10: 23409, pivots: { pdh: 23461.5, pdl: 23302.1, pivot: 23392.6, r1: 23483.2, r2: 23552, r3: 23642.6, s1: 23323.8, s2: 23233.2, s3: 23164.4 }, series });
  const briefs = [{ id: uid(), user_id: USER.id, date: today(), as_of: addDays(today(), -1), updated_at: new Date().toISOString(),
    ...
    stocks: [['RELIANCE', 1412, 1.4, 61, 'up'], ['HDFCBANK', 968, -0.4, 47, 'sideways'], ['ICICIBANK', 1421, 0.6, 55, 'up'], ['INFY', 1512, 1.9, 63, 'up'], ['TCS', 3090, 1.1, 46, 'sideways'], ['BHARTIARTL', 1930, 0.3, 58, 'up'], ['ITC', 411, -0.7, 42, 'down'], ['LT', 3660, 0.9, 60, 'up'], ['SBIN', 858, -0.2, 52, 'sideways'], ['AXISBANK', 1105, -1.3, 38, 'down'], ['KOTAKBANK', 2010, 0.1, 50, 'sideways'], ['HINDUNILVR', 2560, -0.9, 36, 'down'], ['BAJFINANCE', 1005, 2.3, 72, 'up'], ['MARUTI', 15840, 1.7, 74, 'up'], ['TATAMOTORS', 712, -2.1, 29, 'down'], ['TATASTEEL', 171, 0.8, 57, 'up'], ['SUNPHARMA', 1620, -0.3, 45, 'sideways'], ['TITAN', 3410, 0.5, 59, 'up'], ['ETERNAL', 331, 3.1, 78, 'up'], ['TRENT', 4590, -1.8, 33, 'down']].map(([sy, c, ch, r, t], i) => ({ ...mk(sy, sy, c, ch, r, t), weight_pct: +(11 - i * 0.45).toFixed(2), points: +(((11 - i * 0.45) / 100) * ch * 234).toFixed(1) })),
    breadth: { advances: 31, declines: 19, above_sma20: 34, rsi_over_70: ['BAJFINANCE', 'MARUTI', 'ETERNAL'], rsi_under_30: ['TATAMOTORS'] },
    technicals: { source: 'TradingView scanner', fetched_at: new Date().toISOString(), close: 23329, change_pct: -0.43,
      summary: { all: -0.47, ma: -0.93, osc: 0, label: 'Sell', ma_label: 'Strong sell', osc_label: 'Neutral' },
      oscillators: [{ name: 'Relative Strength Index (14)', value: 34.88, action: 'Neutral' }, { name: 'Stochastic %K (14, 3, 3)', value: 25.43, action: 'Neutral' }, { name: 'Commodity Channel Index (20)', value: -69.72, action: 'Neutral' }, { name: 'MACD Level (12, 26)', value: -240.36, action: 'Sell' }, { name: 'Momentum (10)', value: -450.15, action: 'Buy' }],
      moving_averages: [{ name: 'Exponential Moving Average (10)', value: 23430.45, action: 'Sell' }, { name: 'Simple Moving Average (20)', value: 23703.17, action: 'Sell' }, { name: 'Simple Moving Average (50)', value: 24045.81, action: 'Sell' }, { name: 'Simple Moving Average (200)', value: 24459.21, action: 'Sell' }],
      pivots: { Classic: { R3: 25844.17, R2: 25063.47, R1: 24571.93, Middle: 24282.77, S1: 23791.23, S2: 23502.07, S3: 22721.37 }, Fibonacci: { R3: 25063.47, R2: 24765.24, R1: 24580.99, Middle: 24282.77, S1: 23984.54, S2: 23800.29, S3: 23502.07 }, Camarilla: { R3: 24295.09, R2: 24223.53, R1: 24151.96, Middle: 24282.77, S1: 24008.84, S2: 23937.27, S3: 23865.71 } },
      pivots_daily: { Classic: { R3: 23642, R2: 23551, R1: 23483, Middle: 23399, S1: 23331, S2: 23247, S3: 23179 } } },
    contribution: { points_up: 42.3, points_down: -128.5, net_points: -86.2, advances: 15, declines: 37 },
    oi: { weekly: { expiry: today(), pcr: 1.1, max_call: 23500, max_put: 23400, call_walls: [[23500, 11.9], [23600, 10.8], [23700, 9.5]], put_walls: [[23400, 16.4], [23300, 16.1], [23000, 12.9]], straddle: 133, expected_move: 105, rows: [[23000, 0.4, 12.9], [23100, 0.5, 9.5], [23200, 2.1, 11.4], [23300, 4.0, 16.1], [23400, 8.6, 16.4], [23500, 11.9, 5.1], [23600, 10.8, 1.2], [23700, 9.5, 0.6], [23800, 8.3, 0.4]] }, monthly: { expiry: addDays(today(), 7), pcr: 0.85, max_call: 24000, max_put: 23000, call_walls: [[24000, 11.7], [23500, 5.7]], put_walls: [[23000, 8.5], [23300, 5.6]], rows: [[23000, 1.4, 8.5], [23200, 1.5, 3.5], [23300, 4.2, 5.6], [23500, 5.7, 5.0], [23700, 3.5, 1.1], [23800, 5.2, 1.6], [24000, 11.7, 5.6]] } },
    sentiment: { score: -16, positive: 42, negative: 58, label: 'Mildly negative — oversold bounce possible', why: 'Crude and US help; flows, rupee, Fed and the downtrend hurt.', sections: [{ title: 'Global cues', score: 1, why: 'crude −4%, US +2%' }, { title: 'Nifty technicals', score: -1, why: 'below all DMAs' }, { title: 'Sectors and the Nifty 50', score: -1, why: '12 of 52 above 20-DMA' }, { title: 'Option chain', score: 0, why: 'pinned 23,300–23,500' }] },
    news: [{ title: 'Crude falls 4% as Saudi exports recover; US–Iran talks possible at UN', source: 'Reuters', score: 2, impact: 'high', note: 'Biggest single positive for India.' }, { title: 'FPIs sold ₹20,974 cr in September; rupee at record low 96', source: 'NSDL', score: -2, impact: 'high' }, { title: 'Fed hiked 25 bp; dots point to one more', source: 'Fed', score: -1, impact: 'med' }, { title: 'Nasdaq +2.4% on chips as yields ease', source: 'CNBC', score: 1, impact: 'med' }],
  ...
  const plans = [
    { id: uid(), user_id: USER.id, date: today(), instrument: 'NIFTY 23250 CE (2-week expiry)', instrument_key: 'NSE_INDEX|Nifty 50', side: 'Buy', entry: 286.69, stop: null, target: 380, qty: 910, strike: 23250, option_type: 'CE', expiry: addDays(today(), 14), iv: 11.25, condition: 'FOMO on crude falling — not a planned trade; averaged with account money', status: 'live', fill: 286.69, sort: 0, note: 'Live-checked: close to the real close today. See the payoff grid before deciding.' },
    { id: uid(), user_id: USER.id, date: today(), instrument: 'LAURUSLABS 1900 CE 29 SEP', instrument_key: 'NSE_FO|DEMO2', side: 'Buy', entry: 123.94, stop: null, target: null, qty: 1680, condition: 'Closed', status: 'done', fill: 123.94, exit: 127.33, sort: 1, note: 'Closed +₹5,695.20' },
  ];
