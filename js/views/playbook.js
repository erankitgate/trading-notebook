...
import { money, cls, pct, rmult, lines, plural, key, arr } from '../lib/fmt.js';
import { bySetup, ruleBreakCounts } from '../lib/stats.js';
...
export function list(ctx) {
  ctx.setNav('playbook');
  const stats = Object.fromEntries(bySetup(S.diary).map((g) => [g.key, g]));
  ...
  for (const s of S.setups) {
    ...
    h += `<article class="card ${s.active === false ? 'inactive' : ''}"><div class="meta">${s.active === false ? 'Paused · ' : ''}${st ? `${plural(st.trades, 'trade')}` : 'no trades yet'}</div><h3>${esc(s.name)}</h3>${s.description ? `<p>${esc(s.description)}</p>` : ''}
      ...
      ${st ? `<div class="mini"><span>Win rate <b>${pct(st.winRate)}</b></span><span>Avg R <b class="${st.avgR == null ? '' : cls(st.avgR)}">${rmult(st.avgR)}</b></span><span>Net <b class="${cls(st.net)}">${money(st.net)}</b></span></div>` : ''}
  ...
  h += `<div class="block"><h2>Hard rules</h2><p class="sub small">These appear as checkboxes on every diary page. Breaking one twice in 30 days puts it in the red box on the front page.</p>`;
  if (S.rules.length) h += `<ul class="rulebook">${S.rules.map((r) => { const b = breaks[key(r.text)]; return `<li class="${r.active ? '' : 'off'}"><span>${esc(r.text)}${b ? ` <span class="mini-flag${b.count >= 2 ? '' : ' amber'}">broken ×${b.count}</span>` : ''}</span><span class="acts"><button type="button" class="btn small ghost" data-toggle="${attr(r.id)}">${r.active ? 'Pause' : 'Resume'}</button><button type="button" class="btn small danger" data-del="${attr(r.id)}" aria-label="Delete rule">×</button></span></li>`; }).join('')}</ul>`;
